...
class Issues extends Component
{
    ...
    // NOTE: Old edit options properties removed - now using paginated dropdowns
    
    // Paginated data fetching methods (same as Slip component)
    #[Renderless]
    public function getPaginatedSlips($search = '', $page = 1, $perPage = 20, $includeIds = [])
    {
        ...
        if (!empty($includeIds)) {
            return ['data' => Vehicle::whereIn('id', $includeIds)->orderBy('vehicle')->get()->pluck('vehicle', 'id')->toArray(), 'has_more' => false, 'total' => count($includeIds)];
        }
    ...
    public function getPaginatedDrivers($search = '', $page = 1, $perPage = 20, $includeIds = [])
    {
        ...
        if (!empty($search)) {
            $query->where(function($q) use ($search) {
                $q->where('first_name', 'like', "%$search%")->orWhere('middle_name', 'like', "%$search%")->orWhere('last_name', 'like', "%$search%");
            });
        }
        
        if (!empty($includeIds)) {
            return ['data' => Driver::whereIn('id', $includeIds)->orderBy('first_name')->orderBy('last_name')->get()->mapWithKeys(fn($d) => [$d->id => trim("{$d->first_name} {$d->middle_name} {$d->last_name}")])->toArray(), 'has_more' => false, 'total' => count($includeIds)];
        }
    ...
    public function getPaginatedGuards($search = '', $page = 1, $perPage = 20, $includeIds = [])
    {
        ...
        if (!empty($search)) {
            $query->where(function($q) use ($search) {
                $q->where('first_name', 'like', "%$search%")->orWhere('middle_name', 'like', "%$search%")->orWhere('last_name', 'like', "%$search%")->orWhere('username', 'like', "%$search%");
            });
        }
        
        if (!empty($includeIds)) {
            return ['data' => User::whereIn('id', $includeIds)->orderBy('first_name')->orderBy('last_name')->get()->mapWithKeys(fn($u) => [$u->id => trim("{$u->first_name} {$u->middle_name} {$u->last_name}") . " @{$u->username}"])->toArray(), 'has_more' => false, 'total' => count($includeIds)];
        }
    ...
    public function getPaginatedLocations($search = '', $page = 1, $perPage = 20, $includeIds = [])
    {
        ...
        if (!empty($includeIds)) {
            return ['data' => Location::whereIn('id', $includeIds)->orderBy('location_name')->get()->pluck('location_name', 'id')->toArray(), 'has_more' => false, 'total' => count($includeIds)];
        }
    ...
    public function openDetailsModal($issueId)
    {
        ...
        $this->selectedIssue = Issue::with([
            ...
            'slip' => function($q) {
                $q->select('id', 'slip_id', 'vehicle_id', 'location_id', 'destination_id', 'driver_id', 'status', 'completed_at', 'deleted_at')->withTrashed();
            },
    ...
    public function resolveIssue()
    {
        ...
        try {
            ...
            $issue = Issue::findOrFail($this->selectedIssue->id);
            $oldValues = [
    ...
    private function getFilteredIssuesQuery()
{
    ...
    $query = Issue::with([
        'user' => function($q) { $q->select('id', 'first_name', 'middle_name', 'last_name', 'username', 'deleted_at')->withTrashed(); },
        'slip' => function($q) { $q->select('id', 'slip_id', 'vehicle_id', 'location_id', 'destination_id', 'driver_id', 'status', 'completed_at', 'deleted_at')->withTrashed(); },
        'resolvedBy' => function($q) { $q->select('id', 'first_name', 'middle_name', 'last_name', 'username', 'deleted_at')->withTrashed(); }
    ])->whereNull('deleted_at');
    
    // ADMIN: Exclude issues with deleted users or deleted slips
    ...
    // Filters
    if (!is_null($this->appliedIssueType) && $this->appliedIssueType !== '') {
    ...
    public function canRemoveAttachment()
    {
        ...
        // Admin cannot remove Photo from completed slips (status == 2 or completed_at is set)
        // Only SuperAdmins can remove photos from completed slips
