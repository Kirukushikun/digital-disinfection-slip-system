    ...
    @endif
        ...
        <button type="button" 
            ...
            <svg xmlns="http://www.w3.org/2000/svg" 
                class="w-5 h-5 ml-2 -mr-1 transition-transform"
        ...
        <div x-show="open" 
            ...
            <div class="overflow-y-auto" style="max-height: {{ $maxHeight }};">
                <template x-for="[value, label] in Object.entries(filteredOptions)" :key="value">
                    <a href="#"
                        @if ($multiple)
                            x-on:click.prevent="updateSelection(Number(value))"
                        @else
                            x-on:click.prevent="
                        ...
                        :class="@if($multiple) localSelection.includes(Number(value)) @else $wire.get('{{ $wireModel }}') == Number(value) @endif ? 'bg-blue-50 text-blue-700' : ''">
                        <div class="flex items-center justify-between">
                            <span x-text="label"></span>
                            @if ($multiple)
                                <svg class="w-5 h-5 text-blue-600" fill="currentColor" viewBox="0 0 20 20" 
                                    x-show="localSelection.includes(Number(value))"
                                    style="display: none;">
                                    <path fill-rule="evenodd"
