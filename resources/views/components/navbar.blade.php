...
>
    ...
    <div class="fixed inset-0 z-40 pointer-events-none" x-cloak>
        <!-- Panel -->
        <div
            class="absolute top-0 left-0 h-full w-72 max-w-full bg-[#ffd4b0] border-r-2 border-black shadow-xl rounded-r-2xl p-6 flex flex-col gap-6 pointer-events-auto"
            x-show="open"
        ...
        >
            ...
            <div class="bg-gray-100/90 shadow-md rounded-md p-4 flex items-center justify-between gap-4">
                <div class="flex flex-col flex-1 min-w-0">
                    ...
                    <span class="text-sm text-gray-500 wrap-break-word mt-0.5">{{ '@' . (auth()->user()->username ?? 'username') }}</span>
                </div>
                <button
                    type="button"
                    class="shrink-0 inline-flex items-center justify-center w-10 h-10 rounded-lg bg-white border-2 border-gray-300 text-gray-700 shadow-sm hover:bg-[#EC8B18] hover:border-[#EC8B18] hover:text-white hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#EC8B18] focus:ring-offset-2 transition-all duration-200 cursor-pointer"
            ...
            <div class="space-y-3">
                ...
                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    ...
                    <button
                        type="submit"
                        class="w-full rounded-full px-3 py-2 text-sm font-semibold text-gray-800 bg-[#FFF7F1] hover:bg-gray-200 hover:border-2 hover:border-[#EC8B18] hover:shadow-md hover:scale-[1.02] active:scale-[0.98] focus:ring-2 focus:ring-[#FFF7F1] transition-all duration-200 cursor-pointer"
                    >
                        Logout
                    </button>
                </form>
