<div>
    <x-modals.modal-template show="showModal" max-width="max-w-md">
        <x-slot name="header">
        ...
        <p class="text-sm text-gray-600">
            @if ($vehicleDisabled)
                Are you sure you want to enable <span class="font-semibold text-gray-900">{{ $vehicleName }}</span>? This vehicle will be
                available for use again.
            @else
                Are you sure you want to disable <span class="font-semibold text-gray-900">{{ $vehicleName }}</span>? This vehicle will not be
                available for use.
            @endif
        </p>
        ...
        <x-slot name="footer">
            ...
            @if ($vehicleDisabled)
                <button wire:click="toggle" wire:loading.attr="disabled" wire:target="toggle"
                    ...
                    <span wire:loading.remove wire:target="toggle">Enable Vehicle</span>
            ...
            @else
                <button wire:click="toggle" wire:loading.attr="disabled" wire:target="toggle"
                    ...
                    <span wire:loading.remove wire:target="toggle">Disable Vehicle</span>
