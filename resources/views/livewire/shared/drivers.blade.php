<div class="min-h-screen bg-gray-50 p-6" @if (!$showFilters && !(($config['showRestore'] ?? false) && ($showRestoreModal ?? false))) wire:poll.keep-alive @endif>
    <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div class="flex gap-3 w-full lg:w-auto">
                    ...
                    @if ($config['showRestore'] ?? false)
                        ...
                        @endif
                    @else
                        {{-- Export Button with Create option (mobile, for Admin/User) --}}
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ (($config['showRestore'] ?? false) && ($showDeleted ?? false)) ? '3' : '4' }}" class="px-6 py-12 text-center">
        ...
        {{-- Edit Modal --}}
        <livewire:shared.drivers.edit :config="['minUserType' => $config['minUserType'] ?? 2]" />

        {{-- Disable/Enable Confirmation Modal --}}
        <livewire:shared.drivers.disable :config="['minUserType' => $config['minUserType'] ?? 2]" />

        {{-- Create Driver Modal --}}
        <livewire:shared.drivers.create :config="['minUserType' => $config['minUserType'] ?? 2]" />

        {{-- Delete Confirmation Modal --}}
        <livewire:shared.drivers.delete :config="['minUserType' => $config['minUserType'] ?? 2]" />
                    ...
                    @csrf
                        ...
                        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                            ...
                            <button wire:click.prevent="restoreDriver" wire:loading.attr="disabled" wire:target="restoreDriver"
                                ...
                                <span wire:loading.inline-flex wire:target="restoreDriver" class="inline-flex items-center gap-2"><svg class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Restoring...
                                </span>
                            </button>
                        </div>
                    </div>
