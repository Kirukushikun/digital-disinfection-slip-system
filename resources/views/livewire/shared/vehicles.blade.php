<div class="min-h-screen bg-gray-50 p-6" @if (!$showFilters && !(($config['showRestore'] ?? false) && ($showRestoreModal ?? false))) wire:poll.keep-alive @endif>
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-6">
            ...
            @elseif (($appliedCreatedFrom || $appliedCreatedTo) && ($showDeleted ?? false) && ($config['showRestore'] ?? false))
                <div class="mt-4 flex flex-wrap gap-2">
        ...
        {{-- Filter Modal --}}
        @if (($showDeleted ?? false) && ($config['showRestore'] ?? false))
        ...
        {{-- Edit Modal --}}
        <livewire:shared.vehicles.edit :config="['minUserType' => $config['minUserType'] ?? 1]" />

        {{-- Disable/Enable Confirmation Modal --}}
        <livewire:shared.vehicles.disable :config="['minUserType' => $config['minUserType'] ?? 1]" />

        {{-- Create Vehicle Modal --}}
        <livewire:shared.vehicles.create :config="['minUserType' => $config['minUserType'] ?? 1]" />

        {{-- Delete Confirmation Modal --}}
        <livewire:shared.vehicles.delete :config="['minUserType' => $config['minUserType'] ?? 1]" />
                ...
                <div class="flex min-h-full items-center justify-center p-4">
                    <div
                        ...
                        <div class="px-6 py-4 bg-white border-b border-gray-200">
                            <div class="flex items-center">
                                ...
                                <h3 class="ml-4 text-lg font-semibold text-gray-900">Restore Vehicle</h3>
                            </div>
                        </div>
                    ...
                    @csrf
                        ...
                        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                            <button wire:click="closeModal" wire:loading.attr="disabled" wire:target="restoreVehicle"
                                ...
                                <button wire:click.prevent="restoreVehicle" wire:loading.attr="disabled" wire:target="restoreVehicle"
                                ...
                                <span wire:loading.remove wire:target="restoreVehicle">Restore Vehicle</span>
                                <span wire:loading.inline-flex wire:target="restoreVehicle" class="inline-flex items-center gap-2"><svg class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Restoring...
                                </span>
                            </button>
                        </div>
                    </div>
