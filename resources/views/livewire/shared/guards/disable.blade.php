<div>
    <x-modals.modal-template show="showModal" max-width="max-w-lg">
        ...
        <p class="text-sm text-gray-600">
            @if ($userDisabled)
                Are you sure you want to enable <span class="font-semibold text-gray-900">{{ $userName }}</span>? This guard will be able to access the
                system again.
            @else
                Are you sure you want to disable <span class="font-semibold text-gray-900">{{ $userName }}</span>? This guard will not be able to access
                the system.
            @endif
        </p>
        ...
        <x-slot name="footer">
            ...
            @if ($userDisabled)
                <button wire:click="toggle" wire:loading.attr="disabled" wire:target="toggle"
                    ...
                    <span wire:loading.remove wire:target="toggle">Enable Guard</span>
            ...
            @else
                <button wire:click="toggle" wire:loading.attr="disabled" wire:target="toggle"
                    ...
                    <span wire:loading.remove wire:target="toggle">Disable Guard</span>
