<div class="min-h-screen bg-gray-50 p-6" @if (!$showFilters && !(($config['showRestore'] ?? false) && ($showRestoreModal ?? false))) wire:poll.keep-alive @endif>
    <div class="max-w-7xl mx-auto">
        ...
        {{-- Old modals removed - using shared components above --}}
        {{-- Previously had inline Edit and Create modals - now using livewire:shared.locations.edit and livewire:shared.locations.create --}}
        @if (false)
                ...
                <div class="flex min-h-full items-center justify-center p-4">
                    ...
                    @csrf
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Allow Create Slip</label>
                                    <label class="relative inline-flex items-center cursor-pointer" x-data="{ createSlip: @entangle('create_create_slip').live }">
                                        <input type="checkbox" wire:model.live="create_create_slip" x-model="createSlip" class="sr-only">
                                        <div class="w-11 h-6 rounded-full focus-within:outline-none focus-within:ring-4 focus-within:ring-blue-300 transition-colors duration-200 relative" :class="createSlip ? 'bg-blue-600' : 'bg-gray-200'">
                                            <div class="absolute top-[2px] left-[2px] bg-white border border-gray-300 rounded-full h-5 w-5 transition-transform duration-200" :class="createSlip ? 'translate-x-5' : 'translate-x-0'"></div>
                                        </div>
                                        <span class="ml-3 text-sm text-gray-700" x-text="createSlip ? 'Enabled - Guards can create slips at this location' : 'Disabled - Guards cannot create slips at this location'"></span>
                        ...
                        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                            ...
                            <button wire:click.prevent="createLocation" wire:loading.attr="disabled" wire:target="createLocation"
                                ...
                                <span wire:loading.inline-flex wire:target="createLocation" class="inline-flex items-center gap-2"><svg class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Creating...</span>
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    @csrf
                        ...
                        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                            ...
                            <button wire:click.prevent="restoreLocation" wire:loading.attr="disabled" wire:target="restoreLocation"
                                ...
                                <span wire:loading.inline-flex wire:target="restoreLocation" class="inline-flex items-center gap-2"><svg class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Restoring...
                                </span>
                            </button>
                        </div>
                    </div>
